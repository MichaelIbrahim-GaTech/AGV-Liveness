...
BlockDirectedAcyclicMultiGraph_3::BlockDirectedAcyclicMultiGraph_3(DirectedAcyclicMultiGraph* _U, set<int> Block, int _parentAP)
...
bool BlockDirectedAcyclicMultiGraph_3::IsThereATerminalNodeThatDoesntHaveMaximalCapacity(CondensedMultiGraph* _C)
{
	directed[parentAP].insert(pair<int, int>(-1, -1));
...
bool BlockDirectedAcyclicMultiGraph_3::ExistAMeregerSequenceForATerminalNode(CondensedMultiGraph* _C, bool& _terminate)
{
	directed[parentAP].insert(pair<int, int>(-1, -1));
...
void BlockDirectedAcyclicMultiGraph_3::Algorithm4_NAHS(CondensedMultiGraph* _C)
{
	vector<int> L;
	set<int> Lset;
	...
	vector<int> Li_part;
	vector<int> M;
	// start from the parent ap instead of searching for a source node. 
	int v = parentAP;
